...
if __name__ == "__main__":
    ...
    while True:
        ...
        data = resp.json()
        with open(f'./result_{page}.json', 'w') as f:
            json.dump(data, f, indent=2)

        # For each page, there are issues in data['results'] as a list.
        for issue in data['results']:
            # Dupe check issue['id']
            if client.indices.exists(index="news_board_doc") and not client.get(index="news_board_doc", id=issue['id']):
                # Each can either be a news or a media(e.g. YT) and has ids and urls.
                client.index(index="news_board_doc", id=issue['id'], document=issue)
                # issue['source']['url'] -> original article handled by the next stage? i.e., logstash? ... 
                # no it has to be crawled. it has to be done here
